...
import * as actions from '../actions/friendApplication';
import {
  howWellKnowStudent,
  churchManagementRelationship,
  personalRecommendation,
} from '../schemas/friendApplication';
...
const initialState = fromJS({
  ...
  friendFeedback: {
    ...
    areasThatNeedDevelopment: undefined,
    involvedInChurch: true,
    ...
    studentTraits: {
      ...
      emotionalStability: percents.p100,
    },
...
const friendApplicationReducer = (state = initialState, action) => {
  switch (action.type) {
  case actions.FRIEND_APPLICATION_STUDENT_NAME_LOADED:
    return state.set('studentName', action.payload);

  case actions.FRIEND_APPLICATION_UPLOADING:
    return state
      .set('friendFeedback', action.payload)
      .mergeIn(['upload'], fromJS({ pending: true, error: false, errorMessage: '' }));

  case actions.FRIEND_APPLICATION_UPLOADED:
    return state.setIn(['upload', 'pending'], false);

  case actions.FRIEND_APPLICATION_UPLOAD_ERROR:
    return state.mergeIn(['upload'], fromJS({ pending: false, error: true, errorMessage: action.payload }));

  case actions.FRIEND_APPLICATION_SHOW_UPLOAD_SUCCESS_ALERT:
    return state.set('showUploadSuccessAlert', true)
      .set('uuid', action.payload);

  case actions.FRIEND_APPLICATION_HIDE_UPLOAD_SUCCESS_ALERT:
...
export default friendApplicationReducer;
